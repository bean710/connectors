...
class OracleQueries(Queries):
    ...
    def table_data(self, **kwargs):
        ...
        if 'timestamp' in kwargs and kwargs['timestamp'] is not None:
            ...
            return f"SELECT * FROM {kwargs['table']} WHERE LAST_UPDATE_DATE >= TO DATE ('{timestamp}', 'DD-MM-YYYY HH24:MI:SS')"
            ...
            if row_count > 0:
                ...
                if keys:
                    ...
                    async for row in streamer:
                        ...
                        doc_update_time = iso_utc(parse_datetime_string(row_time))
                        keys_value = ""
                        for key in keys:
